...
public class GameScreen implements Screen {
    ...
    private void onBoardClick(int x, int y) {
        ...
        if (selectedPiece != null) {
            ...
            if (chosen != null) {
                ...
                boolean moved = gameManager.movePiece(
                        chosen.startX,
                        chosen.startY,
                        chosen.endX,
                        chosen.endY
                ...
                if (moved) {
                    ...
                    checkGameEnd(); // vérifie mat ou pat
    ...
    private void checkGameEnd() {
        ...
        if (b.isCheckmate(true)) {
            ...
            game.setScreen(new EndScreen(
                    game,
                    "Echec et mat - Noir a gagné",
                    gameManager.getWhiteCapturedPawns(),
                    gameManager.getBlackCapturedPawns()
            ));
        }
            ...
            game.setScreen(new EndScreen(
                    game,
                    "Echec et mat - Blanc a gagné",
                    gameManager.getWhiteCapturedPawns(),
                    gameManager.getBlackCapturedPawns()
            ));
        }

        else if (b.isStalemate(true) || b.isStalemate(false)) {
    ...
    private void drawCheckOutlines() {
        ...
        if (b.isInCheck(true)) {
            ...
            if (king != null)
                drawRedKingOutline(king.getX(), king.getY());
        }
    ...
    private Piece findKing(boolean white) {
        ...
        for (int x = 0; x < BOARD_SIZE; x++) {
            ...
            for (int y = 0; y < BOARD_SIZE; y++) {
                ...
                if (p != null &&
                    p.isWhite() == white &&
                    "King".equals(p.getClass().getSimpleName())) {
    ...
    private Texture getTextureForPiece(Piece piece) {
        ...
        switch (name) {
            ...
            case "Pawn":
                return isWhite ? assets.wPawn : assets.bPawn;

            case "Rook":
                return isWhite ? assets.wRook : assets.bRook;

            case "Knight":
                return isWhite ? assets.wKnight : assets.bKnight;

            case "Bishop":
                return isWhite ? assets.wBishop : assets.bBishop;

            case "Queen":
                return isWhite ? assets.wQueen : assets.bQueen;

            case "King":
                return isWhite ? assets.wKing : assets.bKing;
